...
class BinanceVisionMetricsMultiDownloader:
    DB_NAME = 'binance_future_metrics'
    ...
    def __init__(self, symbols: List[str], start_date: str, end_date: str,
        ...
        self.base_url = "https://data.binance.vision/?prefix=data/futures/um/daily/metrics/"
    ...
    async def setup_timeseries_collection(self, symbol: str):
        ...
        collection = f"{symbol.lower()}_{self.COLLECTION_NAME}"
    ...
    async def update_metadata(self, symbol: str, min_timestamp: datetime, max_timestamp: datetime):
    ...
    def _get_file_url(self, symbol: str, date: datetime) -> str:
        ...
        filename = f"{symbol}-{self.COLLECTION_NAME}-{date_str}.zip"
    ...
    def _get_checksum_url(self, symbol: str, date: datetime) -> str:
        ...
        date_str = date.strftime("%Y-%m-%d")
        filename = f"{symbol}-{self.COLLECTION_NAME}-{date_str}.zip.CHECKSUM"
        return f"{self.base_url}/{symbol}/{filename}"
                        ...
                        with zipfile.ZipFile(io.BytesIO(content)) as zf:
                            ...
                            with zf.open(csv_filename) as csv_file:
                                ...
                                df = pd.read_csv(csv_file, header=None, skiprows=1, names=[
                                    'open_time', 'symbol', 'sum_open_interest', 'sum_open_interest_value',
                                    'count_top_traders_long_short_ratio', 'sum_top_traders_long_short_ratio',
    ...
    async def process_symbol(self, symbol: str) -> Dict[str, Any]:
        ...
        if min_timestamp and max_timestamp:
            await self.update_metadata(symbol, min_timestamp, max_timestamp)
...
async def main():
    ...
    symbols = ["BTCUSDT"]
    downloader = BinanceVisionMetricsMultiDownloader(
        symbols=symbols,
