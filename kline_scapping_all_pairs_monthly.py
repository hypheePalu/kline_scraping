...
class BinanceVisionKlinesMultiDownloader:
    DB_NAME = 'binance_future_klines'
    ...
    def __init__(self, symbols: List[str], start_date: str, end_date: str,
        ...
        # Setup MongoDB
        load_dotenv()
        mongo_uri = f"mongodb+srv://{os.getenv('MONGO_USER')}:{os.getenv('MONGO_PASSWORD')}@{self.DB_NAME}/{os.getenv('MONGO_NAME')}?tls=true&authSource=admin&replicaSet=db-mongodb-sgp1-43703"
    ...
    async def setup_timeseries_collection(self, symbol: str):
        ...
        collection = f"{symbol.lower()}_{self.interval}_klines"
...
async def main():
    ...
    symbols = ["BTCUSDT"]
    downloader = BinanceVisionKlinesMultiDownloader(
        symbols=symbols,
